...
pdf.options(paper = "a4") #makes sure plots are saved in A4 paper 
```
...
# using chr6_bmap_all, code to count number of cis, inter and intra interactions (for proportion of interactions by type)
```{r}
...
#functions to read desired chr data from GTEx and codes3D
```{r}
...
#gene ref table from gtex specifying pos of genes
```{r}
#gtex gtf table
#Remove the Y chr since it has gene names same as those in X axis but they have different positions
<<<<<<< HEAD
gene_ref<-read.table("./data/gencode.v26.GRCh38.genes.gtf.gz", sep="\t") %>% 
=======
gene_ref<-read.table("./data/gencode.v26.GRCh38.genes.gtf", sep="\t") %>% 
>>>>>>> b3798f509b9097137cf682f9fea3072d727298a8
...
#plot gene density in whole blood 
```{r}
...
#Chr6 read data across all tissues
```{r}
...
#CHR19 read data across all 3 tissues
```{r}
...
#CHR7 read data across all 3 tissues
```{r}
...
#CHR18 read data across all 3 tissues
```{r}
...
#function to bin chromosome into 1Mb bins, count number of tissue_map eQTLs 
```{r}
...
#MANUAL BINNING AND COMPUTE RATIO
#CHR6
```{r}
