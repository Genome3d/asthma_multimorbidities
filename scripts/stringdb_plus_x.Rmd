...
#read bmap data
```{r}
bmap<-vroom("data/blood_grn_sig.txt", delim="\t", col_select = c("gencode_id", "snp", "gene", "interaction_type", "eQTL_eGene")) %>% as.data.frame() %>%
...
```{r}
gwas_catalog<- read.csv("./data/gwas_catalog_v1.0.2-associations_e100_r2021-05-19.tsv", sep = "\t",  stringsAsFactors=F) %>% dplyr::select("DISEASE.TRAIT", "REPORTED.GENE.S.", "MAPPED_GENE", "P.VALUE", "STUDY.ACCESSION" , "SNPS") %>% dplyr::rename("snp"=SNPS)
```
#383 asthma SNPs
```{r}
